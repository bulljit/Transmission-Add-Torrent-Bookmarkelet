...
(function(){
	...
	for(var i=0,j=a.length;i<j;i++){
		...
		if (testResult == -1){
			var testResult = linkurl.search(/(magnet:\?|\/get|\/download|\/dl)/i);
		}
		
		if (testResult != -1){
			a[i].setAttribute('target','_blank');
			a[i].setAttribute('href',"http://" + serverAddress + "/transmission/web/fetchtorrent.html?torrentlink=" + encodeURIComponent(linkurl) + "&cs=" + encodeURIComponent(cookieString));
